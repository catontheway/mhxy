...
BOOL cItem::UseItem22(sItem2* pItem, int ObjId)
{
	...
	case 59://八珍玉液
		success = TRUE;
		AddAll(&cItem::AddHpHurt, 30, ObjId, 10000);
		AddAll(&cItem::AddHp, 30, ObjId, 10000);
		AddAll(&cItem::AddMp, 30, ObjId, 10000);
		break;
	case 60://千金露
		success = TRUE;
...
BOOL cItem::UseItem27(sItem2* pItem, int ObjId,  int* num)
{
	...
	case 5:
		
		{
			...
			sAddPoint addpoint;
			addpoint.Set(4, 0, 0, 1, 0);//pPet1->m_PetDataZZ.modeid
			if (rand() % 100 < 50)
			{
				g_pMainState->MakeAPet(pPet1, pPet1->m_PetDataZZ.modeid, 100, 0, addpoint);//珍品
				g_pMainState->m_Tags.Add("恭喜，你洗练出了珍品宠物");
			}
			else
				g_pMainState->MakeAPet(pPet1, pPet1->m_PetDataZZ.modeid, 80, 0, addpoint);//非珍品
			(*num) -= 1;
		}
		return TRUE;
	case 36://碧藕
	...
	case 63:
		if (1)
		{
			...
			if (g_pMainState->m_FightList[g_pMainState->m_HeroID + 5].isLive)
			{
				...
				pPet->m_XiuLian.Set(pPet, g_pCharacter->m_PCData.m_BBXiuLian);
			}
		}
		return TRUE;
	case 64:
...
{
	if (pItem->GetNum() == 0)return FALSE;
...
void cItem::SetFabaoEffect(cPCData& data, int leveup, eFaBao ID)
{
	...
	case eFaBao17周杰伦手办:
		...
		break;
	case eFaBao18时之环:
		data.m_SPD.AddDataAdd(100 * add);
		data.GoldLivePercent.d1 += 10*add;
		break;
	case eFaBao19通灵宝玉:
	...
	case 6://宝图
		if (1)
		{
			...
			if (abs(pUser->GetY() - pItem2->m_Property[4])<100)
			{
				...
				{
					int money = pc->m_PCData.m_LV * 5000 + 100000;
					oss.str("");
					oss << "你得到了";
					oss << money;
					oss << "钱";
...
void cItem::AutoItemProperty(sItem2* item, int Luck,int dustbin)
{
	...
	case 14:
		if (1)
		{
			...
			property.push_back((5 + Offset * 58)+(Offset * 16 * rand()%(1000+Luck))/2000 );
			property.push_back(eProPerty灵力);
			property.push_back((5 + Offset * 13) + ((Offset * 3 + 3) * rand() % (1000 + Luck)) / 2000);
	...
	case 21:
	
		{
			...
			case 16:
				...
				property.push_back((5 + Offset * 9) + ((Offset * 2 + 3) * rand() % (1000 + Luck)) / 2000);
			...
			case 18:
				...
				property.push_back(eProPerty防御);
				property.push_back((3 + Offset * 14) + ((Offset * 4 + 1) * rand() % (1000 + Luck)) / 2000);
				break;
			case 19:  //项链
				property.push_back(extranum+2);
				property.push_back(eProPerty灵力);
				property.push_back((5 + Offset * 16) + (4 * (Offset + 1)* rand() % (1000 + Luck)) / 2000);
				property.push_back(eProPerty躲避);
				property.push_back((4 + Offset * 8) + ((Offset * 2 + 3) * rand() % (1000 + Luck)) / 2000);
				break;
			case 20: //腰带
				property.push_back(extranum+2);
				property.push_back(eProPerty防御);
				property.push_back((5 + Offset * 5) + ((Offset + 4)* rand() % (1000 + Luck)) / 2000);
				property.push_back(eProPerty气血);
				property.push_back((4 + Offset * 69) + (20 * Offset * rand() % (1000 + Luck)) / 2000);
				break;
			case 21: //鞋
				property.push_back(extranum+2);
				property.push_back(eProPerty躲避);
				...
				property.push_back((4 + Offset * 6) + ((Offset * 5 + 2) * rand() % (1000 + Luck)) / 2000);
				break;
			}
			AutoItemProperty2(property, extranum, Luck, Offset);
	...
	case 34:
		{
			...
			case 33://项圈 速度	
				property.push_back(eProPerty速度);
				property.push_back(3 + Offset * 2 + (3 * (rand() % (100 + Luck / 10))) / 100);

				property.push_back(eProPerty灵力);
				property.push_back(2 + Offset * 5 + (5 * (rand() % (100 + Luck / 10))) / 100);
				break;
			case 34://铠甲 防
				property.push_back(eProPerty防御);
				property.push_back(10 + Offset * 2 + (5 * (rand() % (100 + Luck / 10))) / 100);

				property.push_back(eProPerty气血);
				property.push_back(30 + Offset * 30 + (50 * (rand() % (100 + Luck / 10))) / 100);
				break;
			}
			AutoItemProperty2(property, extranum, Luck, Offset);
		}
...
void cItem::MakePropString2(sItem2* item)
{
	...
	{
		m_Color[m_numtext] = RGB(0, 245, 255);
		oss.str("");
		oss << "附加特技:";
	...
	{
		m_Color[m_numtext] = RGB(132, 112, 255);
		oss.str("");
		oss << "符石效果:";
	...
	m_Color[m_numtext] = RGB(255, 99, 71);
	m_text[m_numtext] = oss.str();
	m_numtext += 1;


	//额外属性
	m_Color[m_numtext] = RGB(154, 255, 154);
	for (int i = 0; i < num; i++)
	{
		if (i % 2 == 0)
		{
...
void cItem::AutoItemProperty2(vector<int>& property, int extranum, int Luck, int Offset)
{
	...
	{
		extrapropnum[i] = rand() % (Offset + Luck / 250 + 30) - Offset / 3;
...
{//宝石效果
	...
	switch (baoshiid)
	{
	case 0:SetUpQiangHua(pro, eProPerty气血, 80*m_bSetup); return;
	case 1:SetUpQiangHua(pro, eProPerty速度, 10 * m_bSetup); return;
	case 2:
		SetUpQiangHua(pro, eProPerty命中, 25 * m_bSetup);
		SetUpQiangHua(pro, eProPerty伤害, 10 * m_bSetup);
		return;
	case 3:SetUpQiangHua(pro, eProPerty火抗, 5 * m_bSetup); return;
	case 4:SetUpQiangHua(pro, eProPerty雷抗, 5 * m_bSetup); return;
	case 5:SetUpQiangHua(pro, eProPerty水抗, 5 * m_bSetup); return;
	case 6:SetUpQiangHua(pro, eProPerty土抗, 5 * m_bSetup); return;
	case 7:SetUpQiangHua(pro, eProPerty灵力, 8 * m_bSetup); return;
	case 8:SetUpQiangHua(pro, eProPerty躲避, 40 * m_bSetup); return;
	case 9:SetUpQiangHua(pro, eProPerty伤害, 12 * m_bSetup); return;
	case 10:SetUpQiangHua(pro, eProPerty防御, 15 * m_bSetup); return;
	case 11:SetUpQiangHua(pro, eProPerty速度, 8 * m_bSetup); return;
	case 12:SetUpQiangHua(pro, eProPerty灵力, 5 * m_bSetup); return;
	case 13:SetUpQiangHua(pro, eProPerty防御, 10 * m_bSetup); return;
	case 14:SetUpQiangHua(pro, eProPerty躲避, 30 * m_bSetup); return;
	case 15:SetUpQiangHua(pro, eProPerty伤害, 15 * m_bSetup); return;
	case 16:SetUpQiangHua(pro, eProPerty气血, 50 * m_bSetup); return;
	}
}
