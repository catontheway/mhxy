...
struct sShangPinPet
{
	...
	void Set(int modeid, sAddPoint* addpoint = 0, int stronglv=80, BOOL needskill = FALSE);
	void AddSkill(int id);
	void Save(ofstream& File);
	void Read(ifstream& File);//从商店读取简化的召唤兽
	void Load(ifstream& File);//从存档读取
...
/*******************Begin 炼妖 合宠**********************/
class cInterfacePet2 :public cInterfaceFather
{
public:
	void OnOff(BOOL NeedShow);
	BOOL   Move(int x, int y);//移动界面
