...
struct sZiZhi
{
	...
	void Set(sZiZhi& zishi, int strong, int limit)
	{
		...
		if (strong > 80)
		{//珍品
			m_ATKZZ = zishi.m_ATKZZ - rand() % (zishi.m_ATKZZ / 72 + 1) * 4;
			m_DEFZZ = zishi.m_DEFZZ - rand() % (zishi.m_DEFZZ / 72 + 1) * 4;
			m_MAGZZ = zishi.m_MAGZZ - rand() % (zishi.m_MAGZZ / 72 + 1) * 4;
			m_HPZZ = zishi.m_HPZZ - rand() % (zishi.m_HPZZ / 72 + 1) * 4;
			m_SPDZZ = zishi.m_SPDZZ - rand() % (zishi.m_SPDZZ / 72 + 1) * 4;
			m_AVOIDZZ = zishi.m_AVOIDZZ - rand() % (zishi.m_AVOIDZZ / 72 + 1) * 4;
			m_Growing = zishi.m_Growing - rand() % (zishi.m_Growing * 100 / 18 + 1) / 100;
		}
		else
		{//非珍品
			m_ATKZZ = zishi.m_ATKZZ - rand() % (zishi.m_ATKZZ / 72 + 1) * 12;//* (60 - rand() % 7) / 60;  //+rand() % (ZiZhiOffset);
			m_DEFZZ = zishi.m_DEFZZ - rand() % (zishi.m_DEFZZ / 72 + 1) * 12;  // + rand() % (ZiZhiOffset);
			m_MAGZZ = zishi.m_MAGZZ - rand() % (zishi.m_MAGZZ / 72 + 1) * 12;  // + rand() % (2 * (ZiZhiOffset));
			m_HPZZ = zishi.m_HPZZ - rand() % (zishi.m_HPZZ / 72 + 1) * 12;  // + rand() % (4 * (ZiZhiOffset));
			m_SPDZZ = zishi.m_SPDZZ - rand() % (zishi.m_SPDZZ / 72 + 1) * 12;  // + rand() % (ZiZhiOffset);
			m_AVOIDZZ = zishi.m_AVOIDZZ - rand() % (zishi.m_AVOIDZZ / 72 + 1) * 12;  // + rand() % (ZiZhiOffset);
			m_Growing = zishi.m_Growing - rand() % (zishi.m_Growing * 100 / 6 + 1) / 100;
		}
	}
...
struct sXiuLian:public sBBXiuLian
{
	void AddXiuLianLv2(int num, eXiuLian type, cPetData* data);
	void Clear(cPetData* data);//将修炼归0 修炼带来的增益也会清除
	void Set(cPetData* data, sBBXiuLian& xiulian);
private:
...
public:
	...
	void ClearXiuLian(){m_XiuLian.Clear(this);}
