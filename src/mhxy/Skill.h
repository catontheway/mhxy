...
const int c_NumPetSkill = 105;        //召唤兽技能的数量
...
inline	void Resurgence(int TargetID);
	...
	void  CountphysicalDamage(int& dam, int UserID, int TargetID, int DamageAdd = 0, int Damageup = 0);
	enum eMagicWuXing
	{
		eMWX无,
		eMWX雷,
	...
	BOOL  CountMagicDamage(int& dam, int UserID, int TargetID, int DamageAdd = 0, int Damageup = 0, eMagicWuXing wuxing = eMWX无);//计算法术伤害
