...
BOOL cTaskBaotu::Process(int stage)
{
	...
	case 0:
		...
		{	
			...
			g_pMainState->m_Dialog.SetObj("左将军王璞");
			...
			oss << "近来传闻#R";
			oss << m_Robber.m_name;
			oss << "#R劫走朝廷运送的镖车,他很可能有一个团伙,我已经打听到他的位置，你速速前去#R";
			...
			cObj* pObj = g_pMainState->FindObjByName("左将军王璞");
	...
	case 1:
	
		{
			...
			sAddPoint point;
			point.Set(0, 0, 0, 5, 0);
			int id = g_pMainState->AddPetEnemy(m_Robber.m_name, m_Robber.m_modeid, g_pCharacter->m_PCData.m_LV + 50, &point, 300);
			cPetData* pPetdata;
	...
	case 3:
		
		{
			if (0==rand()%1)
			{
				...
				g_pMainState->m_Map.AutoGetPos(m_Robber.m_x, m_Robber.m_y, m_Robber.m_mapname);
				ostringstream oss;
				oss << "别打了,我只是个小喽啰#R，您只要抓住我们老大就能拿到各种数不清的宝贝！求求你放过我吧！他们藏在";
				oss << m_Robber.m_mapname;
				oss << "#R " << "#R" << m_Robber.m_x << " " << m_Robber.m_y << "#R""（如果找不到人请重新进一次地图刷新）";
				g_pMainState->FindObjByName(nowname)->Talk(oss.str());
				g_pMainState->m_Dialog.m_pObj = 0;
				g_pMainState->m_Map.RemoveNpc(nowname);
				m_Robber.m_modeid = 148;
			...
			INT64 petexp = 0;
			int money = g_pCharacter->m_PCData.m_LV * 80 + 10000 + rand() % 1000;
			if (lis.isLive&&!lis.CheckDie())
			{
				cPetData* petdata;
				lis.m_pData->GetDateP(petdata);
				petexp = GetNormalFightExp(g_pCharacter->m_PCData.m_LV * 2, g_pCharacter->m_PCData.m_LV * 2);
			}
	...
	case 4:
		...
		{
			cPetData* pPet;
			pPet = AddPCPetEnemyF(g_pCharacter->m_PCData.m_LV * 250, (ePetID)m_Robber.m_modeid, g_pCharacter->m_PCData.m_LV + 30, eMP_随机, m_Robber.m_name);
			pPet->m_Combo.d1 += 100;
			pPet->SetJinJie(1);
			pPet->m_MagicNumAdd += 1;
			for (int i = 0; i < 7; i++)
			{
				pPet = AddPCPetEnemyF(g_pCharacter->m_PCData.m_LV * 60, e强盗, g_pCharacter->m_PCData.m_LV+15 );
...
BOOL cTaskNormal::Process(int stage)
{
	...
	case 4://奖励
		{
			...
			int money = g_pMainState->m_Map.m_EnemyLv*(fightnum+3)*5;
	...
	case 12://南极仙翁
	
		{
			...
			oss << "当前仙灵店铺等级为 ";
			oss << lv;
			oss << " 如果升级需要收您80万大洋，可以吗？";
			g_pMainState->m_TriggerDialog.AddSelect(this, oss.str(), g_pMainState->m_pNowSelect->m_TrueName.GetString(), -1, "我要升级", 13, "滚吧死老头", -1);
		}
		break;
	case 13:
		
		{
			int moneyneed = 800000;
	...
	case 3:
		
		{
			...
			if (m_HuanShu==10)
			{
				...
				int lv=-1;
				if (g_pCharacter->m_PCData.m_LV > 30)
				{
					switch (rand()%50)
					{
					case 0:
...
BOOL cTaskZhuaGui::ReadyToFight(sNpc& npc)
{
	...
	{
	case 26://鬼将
		pPetdata->m_HPMax.AddDataAddPercent(3000);
		pPetdata->m_CounterRate.d1 += 60;
		pPetdata->AddSkill(ePS_高级强力);
		break;
	case 28://吸血鬼
		pPetdata->m_DamageAddPercentMagic.d1 += 50;
		pPetdata->m_HPMax.AddDataAddPercent(1000);
		pPetdata->AddSkill(ePS_高级强力);
		pPetdata->AddSkill(ePS_高级吸血);
		pPetdata->m_SuckBloodRate.d1 += 80;
		break;
	case 10://鬼魂
		pPetdata->m_HitRate.d2 += 60;
		pPetdata->m_HPMax.AddDataAddPercent(1000);
		pPetdata->AddSkill(ePS_高级强力);
		pPetdata->AddSkill(ePS_高级再生);
		pPetdata->AddSkill(ePS_高级敏捷);
		break;
	case 27://幽灵
		pPetdata->m_DamagePercent.d2 += 70;
		pPetdata->m_HPMax.AddDataAddPercent(1000);
		pPetdata->AddSkill(ePS_高级毒);
		pPetdata->AddSkill(ePS_善恶有报);
		break;
	default: //野鬼
		pPetdata->m_DamagePercent.d2 += 70;
	...
	for (int i = 0; i < enemynum; i++)
	{
		...
		enemyname += "大恶鬼";
		int id = g_pMainState->AddPCPetEnemy((ePetID)modeid, g_pCharacter->m_PCData.m_LV - 5 , &addpoint, (eMengPai)(rand() % 12), enemyname);
		g_pMainState->m_FightList[id].m_pData->GetDateP(pPetdata);
		pPetdata->FillHPMP();
	}
	g_StateType = STATE_FIGHT;
...
void cTaskZhuaGui::AutoGetGost(sNpc& npc, string mapname)
{
	...
	switch (rand() % 5)
	{
	case 0:modeid = 46; break;//血鬼
	case 1:modeid = 47; break;//马面
	case 2:modeid = 90; break;//敏鬼
	case 3:modeid = 45; break;//牛头
	...
	case 1://拜师成功
		
		{
			if (0==g_pMainState->m_Dialog.m_retSelect)
			{
				...
				{
				case eMP_大唐官府:
					g_pCharacter->m_PCData.m_Spurting.d1 += 30;
					text += "溅射伤害+30%";
					break;
				case eMP_化生寺:
					g_pCharacter->m_PCData.m_CureCriRate.d1+=10;
					text += "治疗有10%几率加倍";
					break;
				case eMP_方寸山:
					g_pCharacter->m_PCData.ghostSpecial.d1 += 100;
					text += "对拥有鬼魂术的敌人伤害+100%";
					break;
				case eMP_女儿村:
				case eMP_五庄观:
					g_pCharacter->m_PCData.m_SPD.AddDataAddPercent(20);
					text += "速度+20%";
					break;
				case eMP_狮驼岭:
				...
				case eMP_阴曹地府:
					...
					g_pCharacter->m_PCData.m_HPMax.AddDataAddPercent(25);
					text += "气血+25%";
					break;
				case eMP_东海龙宫:
				...
				case eMP_普陀山:
					...
					break;
				case eMP_天宫:
					g_pCharacter->m_PCData.m_HitRate.d2 += 40;
					text += "躲避率+40%";
					break;
				}
...
void cTaskFB::ProcessSJ(int stage)
{
	...
	case 5://得到抓鬼任务
		if (!g_pMainState->m_Dialog.m_retSelect)
		{
			...
			xerr = m_List2[0].m_Npc.m_x;
			yerr = m_List2[0].m_Npc.m_y;
			ostringstream oss;
...
{
	int enemynum =  g_pMainState->AtutoGetEnemyNum();
	if (enemynum > 90)enemynum = 90;
	int lv = g_pCharacter->m_PCData.m_LV + 15;
	sZiZhi zizhi;
	zizhi.Set(2000 + m_HuanShu * 80, 1500 + m_HuanShu * 25, 6000 + m_HuanShu * 150, 4000 + m_HuanShu * 50, 1200 + m_HuanShu * 50, 1200 + m_HuanShu * 50, 160 + (m_HuanShu >> 2));	
	AddPCEnemyF(lv * 250, list.m_Npc.m_modeid, &zizhi, -1, mp, list.m_Npc.m_name, list.m_Npc.weapontype, list.m_Npc.weaponid);
	int petrand;
	for (int i = 0; i < enemynum; i++)
	{
		petrand = 30 + m_HuanShu;
	 	petrand= rand() % petrand+(m_HuanShu>>1);
		while (petrand>93)
		{
			petrand -= rand()%80;
		}
		AddPCPetEnemyF(lv*150,(ePetID)petrand);
	}
	SetMusic();
	g_StateType = STATE_FIGHT;
}
