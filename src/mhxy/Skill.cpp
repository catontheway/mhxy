	...
	if (0 == stage)
	{
		if (m_pFightState->m_pList[UserID]->m_pData->m_NowPos == POS_FIGHT_STAND)
		{
			...
			if (m_bmiaoshushow)
			{
				...
				m_pFightState->m_MiaoShu.SetTime(200);
				...
				for (int i = 0; i < 2; i++)
				{
					if (g_pMainState->m_FightList[TargetID].CheckDie())
					 ...
					 if (jianshetarget>-1)
					 {
						 ...
						 {
							 dam = (dam*rate)/100;
							 AddHp(jianshetarget,-dam);
							 SetDamage(dam, jianshetarget);
...
BOOL cSkillManager::SetDamage(int HP, int Who)
{
	...
	{
		if (color==1)
			pUserList->m_Damage[i].SetColor(RGB(255, 0, 0));
		else
			pUserList->m_Damage[i].SetColor(RGB(0, 128, 128));
	}
	ostringstream oss;
	int num = 0;
	oss.str("");
	...
	oss.str("");
	oss << num;
	pUserList->m_Damage[4].SetString(oss.str());
	HP -= num * 10000;
	num = HP / 1000;
	oss.str("");
	oss << num;
	pUserList->m_Damage[3].SetString(oss.str());
	HP -= num * 1000;
	num = HP / 100;
	oss.str("");
	oss << num;
	pUserList->m_Damage[2].SetString(oss.str());
	HP -= num * 100;
	num = HP / 10;
	oss.str("");
	oss << num;
	pUserList->m_Damage[1].SetString(oss.str());
	HP -= num * 10;

	num = HP;
	oss.str("");
	oss << num;
	pUserList->m_Damage[0].SetString(oss.str());
...
//	int mpmax = pUserData->m_MPMax.GetData();
	...
	if (!bCost)
	{
		...
		case 97: // 烟雨
			...
			{
				if (hp > ((hpmax * 50) / 100))return TRUE;
				else return FALSE;
			}
			break;
		case 98: // 缥缈式
	...
	switch (sSkillID)
	{
	case 0:hpsub = hpmax / 10; break;//横扫消耗
	case 5:cpsub = 150; break;//万剑归宗
	case 1:
	case 1010:
		hpsub = hp / 20; break;
	case 2:mpsub = 75; break;
	case 3:hpsub = hp / 20; break;
	case 4:mpsub = 100; break;
	case 6:mpsub = 50; break;
	case 10:mpsub = 200;; break;
	case 1017:hpsub = hp / 2; break;
	case 11:mpsub = 50; break;
	case 13:mpsub = 30; break;
	case 14:mpsub = 40; break;
	case 15:mpsub = 40; break;
	case 16:mpsub = 40; break;
	case 17:mpsub = 100; break;
	case 18:mpsub = 60; break;
	case 19:mpsub = 60; break;
	case 20:mpsub = 60; break;
	case 21:mpsub = 100; break;
	...
	case 61:mpsub = 20 * num; break;
	case 62:mpsub = 80 * num; break;
	...
	case 98:mpsub = 75; break;//缥缈式
	case 9:hpsub = hp / 20; break;//破釜沉舟新
	...
	case 2103: mpsub = 30 + Skilllv/3; break;//上古灵符
		//
	case 2092: mpsub = 30; break;
	case 2093: mpsub = 80; break;
	case 2094: mpsub = 50; break;
	case 2095: mpsub = 50 + pUserData->m_LV / 2; break;
...
BOOL cSkillManager::Use50(int Skilllv, int sSkillID, int UserID, int TargetID)
{
	...
	case 5:  //攻击判定
		
		{
			...
			{
			case 50:
				CountphysicalDamage(dam, UserID, TargetID, 0, 0);
				break;
			case 2095://力劈
				CountphysicalDamage(dam, UserID, TargetID, 0, 0);
				break;
...
//	int mpmax = pUserData->m_MPMax.GetData();
		...
		
		{
			...
			case 3044://破血EX
				
				{
					...
					//case 5:pUserList->m_MaxTargetNum = 5; break;  //万剑归宗
					case 5://万剑归宗
						
						{
							if (hp < ((hpmax * 20) / 100))pUserList->m_MaxTargetNum = 9;
							else
							if (hp < ((hpmax * 40) / 100))pUserList->m_MaxTargetNum = 8;
							else
							if (hp < ((hpmax * 60) / 100))pUserList->m_MaxTargetNum = 7;
							else
							if (hp < ((hpmax * 80) / 100))pUserList->m_MaxTargetNum = 6;
							else
								pUserList->m_MaxTargetNum = 5;
						}
						break;
					...
					case 148:
						
						{
							...
							if (Skilllv < 70)pUserList->m_MaxTargetNum = 2;
							else
							if (Skilllv < 105)pUserList->m_MaxTargetNum = 2;
							else
							if (Skilllv < 140)pUserList->m_MaxTargetNum = 2;
							else
			...
			case 9://破釜沉舟新
				
				{
					...
					case 1017:
						...
						break;
					case 61://雨落寒沙
						if (Skilllv > 89)maxnum = 4;
						else 	if (Skilllv > 59)maxnum = 3;
					...
					case 71://天雷斩
					
						{
							...
							{
								if (Skilllv > 119)maxnum = 5;
								else 	if (Skilllv > 89)maxnum = 4;
								else 	if (Skilllv > 59)maxnum = 3;
								else 	if (Skilllv > 29)maxnum = 2;
								else  	maxnum = 3;
								break;
	...
	case 5:
		if (m_pFightState->WaitPosEnd(UserID) || pUser->m_NowPos == POS_FIGHT_STAND)
		{
			...
			if (pUserList->m_NowTargetID == -1){ pUserList->m_pData->Set(POS_FIGHT_STAND); stage = 7; return TRUE; }
...
BOOL cSkillManager::Use80(int Skilllv, int sSkillID, int UserID, int TargetID)
{
	...
	case 0:
		...
		case 131://飞沙走石
			...
			m_pFightState->m_IDtoLoad[1] = 0x47FE2cc6;//0x49CBD108;
			break;
		default:
			ERRBOX;
			break;
...
BOOL cSkillManager::CheckFightState(int FightListID)
{
	...
	for (list<sBuff>::iterator Iterator = pTarget->m_StateList.begin(); Iterator != pTarget->m_StateList.end();)
	{
		...
		{
					 int autoSelect = g_pMainState->m_AI.AutoFindTarget(FightListID, 4);
					 if (-1 != autoSelect)
					 {
						 m_pFightState->m_pList[FightListID]->m_WorkType = WORKTYPE_MAGIC;
						 //m_pFightState->m_pList[FightListID]->m_SkillSelect.Lv = 0;
						 //m_pFightState->m_pList[FightListID]->m_SkillSelect.SkillID = 1000;
						 m_pFightState->m_pList[FightListID]->targetID = autoSelect;
						 m_pFightState->m_pList[FightListID]->m_SkillSelect.TargetID = autoSelect;
...
//	int  magicType = 1; //默认法术类型 封印
	...
	case 4://杀气诀
		...
		MagicID = 0x85B3D6F1;
		MagicPos = -10;
		pTargetData->m_CounterRate.d1 += 45;
		pTargetData->m_ParryRate.d1 += 20;
		break;
	case 6://反间计
	...
	case 10://长驱直入
		MagicPos = -10;
		MagicID = 0xA0FDF941;
		pTargetData->m_DamageAddPercentCure.d2 -= 50;
		break;
	case 14://催眠
		MagicPos = -10;
		TargetFightP.cannotAll += 1;
	...
	case 26://虚幻咒
		...
		pTargetData->m_HitRate.d2 += 30;
		pTargetData->m_MagicHitRate.d2 += 30;
		TargetFightP.cannotSkill += 1;
		TargetFightP.cannotAttack += 1;
		break;
	case 27: //碎甲符
	...
	case 62:// 天神护体
		...
		MagicUpOrDown = 1;
		MagicPos = 10;
		pTargetData->m_Wakan.AddDataAdd(userlv * 3 / 2);
		break;
	case 63://天神护法
		break;
	case 67:	//镇妖
	...
	case 123://修罗隐身
		...
		{
			pTarget->SetShadow(80);
			pTargetData->m_HitRate.d1 += 10;
			pTargetData->m_HitRate.d2 += 30;
			pTargetData->m_MagicHitRate.d2 += 10;
		}
		break;
	case 125:
	...
	case 1004://保护
		break;
	case 1050://满天花雨普攻
		pUserData->poisonRate.d1 += 25;
		pUserData->m_ATK.AddDataAddPercent(20); 
		break;
	case 2092://惊心一剑
		pTargetData->m_ATK.AddDataAddPercent(-20);
		break;
	...
	case 2085://奔雷咒
		...
		break;
	case 2086://落岩
		pTargetData->m_SPD.AddDataAddPercent(-5);//速度
		break;
	case 2087://泰山压顶
		pTargetData->m_SPD.AddDataAddPercent(-5);//速度
		break;
	case 2088://水攻
		pTargetData->m_Wakan.AddDataAddPercent(-5);//
		break;
	case 2089://水漫
		pTargetData->m_Wakan.AddDataAddPercent(-5);//
		break;
	...
	case 2104://须弥真言
		...
		MagicPos = -10;
		MagicUpOrDown = TRUE;
		pTargetData->m_Wakan.AddDataAdd(15+userlv*2); //灵力
		break;
	case 3024://太极护法 
	case 3025://罗汉
		MagicPos = -10;
...
BOOL cSkillManager::SetDateBuffDown(list<sBuff>::iterator& sbuf)//int targetID, int stateid)
{
	...
	case 1004:
		break;
	case 1050:  //满天花雨普攻
		pUserData->poisonRate.d1 -= 50;
		pUserData->m_ATK.AddDataAddPercent(-30); 
		break;
	case 2092://惊心一剑
		pTargetData->m_ATK.AddDataAddPercent(20);
		break;
	...
	case 2103://上古灵符
		...
		break;
	case 2104://须弥真言主动
		pTargetData->m_Wakan.AddDataAdd(-15-userlv * 2);
		break;
	case 3024://太极护法 罗汉
	case 3025:
		pTargetData->m_DamageAddPercentMagic.d1 -= 50;
...
BOOL cSkillManager::Use1003(int Skilllv, int sSkillID, int UserID, int TargetID)
{
	...
	case 1:
		...
		{
			stage += 1;
			g_pMainState->m_ItemManage.UseItem(GetItem(UserID), TargetID);
...
BOOL cSkillManager::SetPetSkill1(cPetData* pPetDate, int ID, int lv)
{
	...
	case 10://夜战
		...
		pPetDate->m_SPD.AddDataAddPercent(lv * 5);
		pPetDate->nightLv += lv*10;
		break;
	case 11://高级夜战
		pPetDate->m_Spurting.d2 += 100;
	...
	case 14://敏锐
		...
		pPetDate->m_MagicHitRate.d1 += lv * 50;
		break;
	case 15://高级敏锐
		pPetDate->m_HitRate.d1 += lv*30;
		pPetDate->m_MagicHitRate.d1 += lv * 80;
		break;
	case 16://再生
		pPetDate->m_LiveHpLv += lv*15;
		break;
	case 17://高级再生
		pPetDate->m_LiveHpLv += lv*30;
		break;
	case 18://冥思
	...
	case 22://不幸
		...
		break;
	case 23://必杀
		pPetDate->m_CriRate.d1 += lv*15;//pPetDate->m_StartMagic.d1 = 145;//开场变身
		break;
	case 24://高级必杀
		pPetDate->m_CriRate.d1 += lv*25;
		break;
	case 25://幸运
		pPetDate->m_CriRate.d2 -= lv*30;
		pPetDate->m_MagicCriRate.d2 -= lv * 30;
		break;
	case 26://高级幸运
		pPetDate->m_CriRate.d2 -= lv*50;
		pPetDate->m_MagicCriRate.d2 -= lv * 50;
		break;
	case 27://神迹
	...
	case 38://高级防御
		...
		break;
	case 39://偷袭  
		pPetDate->m_StabRate.d2 += lv * 25;//反震免疫率
		pPetDate->m_CounterRate.d2 += lv * 25;//反击免疫率
		pPetDate->m_ParryRate.d2 += lv * 25;//反格档
		break;
	case 40://高级偷袭
	...
	case 74://移花接木 
		...
		break;
	case 75://风刃
		pPetDate->m_Spurting.d1 += lv * 25;
		break;
	case 76://嗜血追击
		pPetDate->ShiXueAdd += lv*5;
		break;
	case 77://龙鳞防御
		pPetDate->m_DamageAddMagicLv.d2 += lv * 2 + 12;//法术伤害结果
	...
	case 80://高级法术抵抗
		...
		break;
	case 81://直死
		pPetDate->m_Pierce.d1+=25;//无视防御率
		pPetDate->m_PierceMagic.d1+=25; //无视灵力率
		break;
	case 82://风华武圣
		pPetDate->comboNumUp += lv*2;
...
BOOL cSkillManager::CheckHit(cPetData* pUserData, cPetData *pTargetData, int offset/*=0*/)
{
	...
	//if (avoid < 1)avoid = 1;
	int hitrate = 95 + pUserData->m_HitRate.d1 - pTargetData->m_HitRate.d2;//offset+10 + pUserData->m_HitRate.d1 - pTargetData->m_HitRate.d2 + (100 * hit) / avoid;
...
BOOL  cSkillManager::CountMagicDamage(int& dam, int UserID, int TargetID, int DamageAdd, int Damageup, eMagicWuXing wuxing)//计算法术伤害
{
	cPetData*  pUserData;
	cPetData*  pTargetData;
	cObj* pUser = m_pFightState->m_pList[UserID]->m_pData;
	...
	int wakanTarget = pTargetData->m_Evade.GetData();//int  wakanTarget = pTargetData->m_Wakan.GetData(); //目标灵力 目标法防
	//wakanTarget -= pUserData->m_LV<<1;//对方灵力降低
	...
	dam = (wakanUser)-(wakanTarget) + DamageAdd;//法术伤害加上技能额外
	BOOL bCri = FALSE;
	if (dam > 1)
	{
		...
		{//法术波动  
			dam = dam * ((100 - pUserData->magicWavePercent) + rand() % (pUserData->magicWavePercent*2)) / 100;
		}
...
void cSkillManager::CountphysicalDamage(int& dam, int UserID, int TargetID, int DamageAdd, int Damageup/*=0*/)
{
	cObj* pUser;
	cObj* pTarget;
	cPetData* UserData;
	...
	dam = (userAtk)-(SufferDef)+DamageAdd;
	//修炼和伤害修正
...
void cSkillManager::EffectSkill(int Skilllv, int sSkillID, int UserID, int TargetID)
{
	...
	case 2095:return;
	case 1050://满天花雨
		CountphysicalDamage(dam, UserID, TargetID, 0, pUserData->m_SPD.GetData()/10 );
		buf.times = 3;//新增毒buff,攻击buff
	...
	case 1095://力劈
		{
			int rate;
			if ( pUserData->m_ATK.GetData() > pTargetData->m_ATK.GetData() )
				rate = 50;
			else
				rate = -50;
			CountphysicalDamage(dam, UserID, TargetID, pUserData->m_ATK.GetData() - pTargetData->m_ATK.GetData(), rate);
		}		
		break;
	case 2101://空间震
		
		{
			...
			CountMagicDamage(dam, UserID, TargetID, 200, 50,eMWX光);
			pUserData->m_PierceMagic.d1 -= 100;
		}
		break;
	case 3040://破碎
	...
	case 3042://破血2
	case 3044://破血EX
		CountphysicalDamage(dam, UserID, TargetID, 0, 10);
		break;
	case 147://定心
		buf.times = 5;
		break;
	...
	case 0://横扫千军
		
		{
			...
			CountphysicalDamage(dam, UserID, TargetID, Skilllv, damageup);
		}
		break;
	case 5: //万剑
		CountphysicalDamage(dam, UserID, TargetID, Skilllv, - 60 + Skilllv / 6);
		break;
	case 1:
		...
		break;
	case 2:// 翩鸿一击
		CountphysicalDamage(dam, UserID, TargetID, Skilllv, 20);
		break;
	case 7:
	
		{
			  CountphysicalDamage(dam, UserID, TargetID, Skilllv, -50 + Skilllv / 6);
		}
		break;
	case 1017:
		CountphysicalDamage(dam, UserID, TargetID, Skilllv, -50 + Skilllv / 6);
	...
	case 8://安神诀
		...
		break;
	case 10:// 长驱直入
		CountphysicalDamage(dam, UserID, TargetID, Skilllv, 50);
		buf.times = 3;
		break;
	case 1010:
		
		{
			CountphysicalDamage(dam, UserID, TargetID, 0, 120);
	...
	case 4://杀气诀
		...
		break;
	case 11://五雷咒
		CountMagicDamage(dam, UserID, TargetID, Skilllv*5, 0, eMWX雷);
		break;
	case 28://落雷符
	
		{
			CountMagicDamage(dam, UserID, TargetID, Skilllv * 5, (10 - m_pFightState->m_pList[UserID]->m_MaxTargetNum) / 10, eMWX雷);
			if (rand() % 5 == 0) buf.times = 2;
		}
		break;
	case 13:
	...
	case 31://唧唧歪歪
	
		{
			...
			CountMagicDamage(dam, UserID, TargetID, Skilllv * 3, (10 - m_pFightState->m_pList[UserID]->m_MaxTargetNum) / 10);
			pUserData->m_Wakan.AddDataOriginal(-pUserData->m_DEF.GetData()>>3);
		}
		break;
	case 33:  //金刚护法
	...
	case 61://雨落寒沙
		...
		dam = Skilllv * 8 + pUserData->m_ProPertySpd *7 / 10;//Skilllv * 9 + (pUserData->m_ATK.GetData()/4);
	...
	case 69:    //百万神兵
		...
		break;
	case 71://天雷斩
		//CountphysicalDamage(dam, UserID, TargetID, Skilllv + pUserData->m_ProPertySpd, 25);
		CountMagicDamage(dam, UserID, TargetID, Skilllv * 5, 0, eMWX雷);
		break;
	case 72://五雷轰顶
		...
		break;
	case 74://雷霆万钧
		CountMagicDamage(dam, UserID, TargetID, Skilllv * 5, 60 + (10 - m_pFightState->m_pList[UserID]->m_MaxTargetNum) / 10, eMWX雷);
		buf.times = 1;
		SetDateBuffUp(UserID, UserID, 0, buf);
		break;
	case 119://寡欲令
	case 77: //解封
		...
		break;
	case 79://龙腾
		CountMagicDamage(dam, UserID, TargetID, Skilllv * 8, 0, eMWX水);
		break;
	case 80://龙卷雨击
		CountMagicDamage(dam, UserID, TargetID, Skilllv * 5, (10 - m_pFightState->m_pList[UserID]->m_MaxTargetNum) / 10, eMWX雷水);
		break;
	case 81://兽王之怒
		buf.times = 10;
		break;
	case 84: //龙呤
		...
		break;
	case 88://二龙
		CountMagicDamage(dam, UserID, TargetID, Skilllv * 7, 0, eMWX水);
		break;
	case 89://日月乾坤
	...
	case 94:   //三花聚顶
		...
		break;
	case 97: //烟雨
		CountphysicalDamage(dam, UserID, TargetID, Skilllv, 0);
		break;
	case 98: //缥缈式
		CountphysicalDamage(dam, UserID, TargetID, Skilllv, 0);
		break;
	case 9: //破釜沉舟新
		{
			...
			CountphysicalDamage(dam, UserID, TargetID, Skilllv, damageup + Skilllv / 15);
		}
		break;
	case 102://普渡众生
		buf.times = 5;
	...
	case 104://杨柳甘露
		...
		break;
	case	105:// 日光华
		dam = Skilllv * 8 + pUserData->m_ProPertySpd * 7 / 10;
		CountMagic2Damage(dam, UserID, TargetID);
		break;
	case	106:// 靛沧海
		dam = Skilllv * 8 + pUserData->m_ProPertySpd * 7 / 10;
	...
	case 115://自在心法
		...
		break;
	case 116://阎罗令
		dam = Skilllv * 7 + pUserData->m_ProPertySpd * 7 / 10;
		CountMagic2Damage(dam, UserID, TargetID);
		AddHpHurt(TargetID, -dam / 10);
		break;
	case 117://判官令
		dam = Skilllv * 10 + pUserData->m_ProPertySpd * 7 / 10;
		CountMagic2Damage(dam, UserID, TargetID);
	...
	case 127:// 魂飞魄散
		...
		break;
	case 131://飞砂走石
		CountMagicDamage(dam, UserID, TargetID, Skilllv * 13/2 + 20, (10 - m_pFightState->m_pList[UserID]->m_MaxTargetNum) / 10, eMWX土);
		break;
	case	132:// 三昧真火
		CountMagicDamage(dam, UserID, TargetID, Skilllv * 6 + 100, 0, eMWX火);
		break;
	case 133://牛劲
		buf.times = 99;
		break;
	case 141://象形
		CountphysicalDamage(dam, UserID, TargetID, Skilllv +20, 0);
		pUser->ReMoveStateID(145);//取消变身//取消状态
		buf.times = 1;
		SetDateBuffUp(UserID, TargetID, sSkillID, buf);
		buf.times = -999;
		break;
	...
	case	145://变身
		...
		break;
	case 148:// 连环击
		CountphysicalDamage(dam, UserID, TargetID, Skilllv, 10 - pUserList->m_NowTarget * 5);
		if (pUserList->m_NowTarget == 0)
		{
			buf.times = 1;
			SetDateBuffUp(UserID, UserID, sSkillID, buf);
	...
	case 152:
		...
		break;
	case 154://勾魂
		dam = -AddHp(TargetID, -Skilllv * 12 - (pTargetData->m_HP / 20));//AddHp就已经造成伤害了 括号里面是负 dam是正
		SetDamage(dam,TargetID);
		dam /= 2;
		CorrectCureHP(dam, UserID, UserID);//治疗修正
		break;
	case 155://摄魄
		dam = -AddMp(TargetID, -userlv * 3 - (pTargetData->m_MP / 20));
		dam /= 2;
	...
	case 161: //复苏
		...
		break;
	case 162:  //天罗地网
		dam = Skilllv * 12 + (pUserData->m_ATK.GetData()/6);
		CountMagic2Damage(dam, UserID, TargetID);
		if (rand() % 2 == 0)
		{
			buf.times = rand() % 3 + 1;
	...
	case 1001://抓捕
		
		{
			...
			if ((rand() % 100) > 50)
			{
				...
				m_pFightState->m_pList[TargetID]->isLive = FALSE;
				m_pFightState->m_pList[TargetID]->SetDie( TRUE);
				pTarget->m_pIndex = 0;
		...
		break;
	case 2084:  //雷击
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 10, eMWX雷);
		if (rand() % 3 == 0) buf.times = 3;
		break;
	case 2085://奔雷咒
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 0, eMWX雷);
		if (rand() % 10 == 0) buf.times = 3;
		break;
	case 2086: //落岩
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 10, eMWX土);
		if (rand() % 3 == 0) buf.times = 3;
		break;
	case 2087: //泰山压顶
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 0, eMWX土);
		if (rand() % 10 == 0) buf.times = 3;
		break;
	case 2088: //水攻
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 10, eMWX水);
		if (rand() % 3 == 0) buf.times = 3;
		break;
	case 2089: //水漫金山
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 0, eMWX水);
		if (rand() % 10 == 0) buf.times = 3;
		break;
	case 2090: //烈火
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 10, eMWX火);
		if (rand() % 3 == 0) buf.times = 3;
		break;
	case 2103: //上古灵符
		if (rand() % 3 == 0)
		{
			m_pFightState->m_pList[pUserList->m_NowTargetID]->SetMagiSufferID(0x3d74be12, 0, &m_pFightState->m_FullSkill.m_SufferMagic[TargetID], 1);
			m_pFightState->m_pList[pUserList->m_NowTargetID]->m_pData->m_MagicSuffer.m_FrameGap = 5;
			CountMagicDamage(dam, UserID, TargetID, userlv * 3, 10, eMWX无);
		}
		else
		{
			m_pFightState->m_pList[pUserList->m_NowTargetID]->SetMagiSufferID(0x1989b002, 0, &m_pFightState->m_FullSkill.m_SufferMagic[TargetID], 1);
			m_pFightState->m_pList[pUserList->m_NowTargetID]->m_pData->m_MagicSuffer.m_FrameGap = 5;
			CountMagicDamage(dam, UserID, TargetID, userlv * 2, 10, eMWX无);
			if (rand() % 10 == 0) buf.times = 2;
		}
		break;
	case 2091: //地狱烈火
		CountMagicDamage(dam, UserID, TargetID, userlv * 2, 0, eMWX火);
		if (rand() % 10 == 0) buf.times = 3;
		break;
	case 2092://惊心一剑
		CountphysicalDamage(dam, UserID, TargetID, userlv, 0);
		buf.times = 1;
		break;
	case 2094://死亡召唤
		CountphysicalDamage(dam, UserID, TargetID, userlv, 0);
	...
	case 2098://善恶有报
		
		{
			...
			if (rand() % 100 > 10+ pTargetData->m_Popularity/100 )
				...
				rate = 30;
			CountphysicalDamage(dam, UserID, TargetID, userlv, rate);
		}
		break;
	case 2099://壁垒击破
		CountphysicalDamage(dam, UserID, TargetID, userlv, 10);
		break;
	case 2100://苍鸾怒击
		CountphysicalDamage(dam, UserID, TargetID, userlv, 30);
		break;
	case 2104:// 须弥真言
		buf.times = 15;
		break;
	case 3000: //气疗
		...
		{
			dam = CountHPPercent(pTargetData, 5, userlv);
			dam += 300;
			CorrectCureHP(dam, UserID, TargetID);//治疗修正
		}
		break;
	case 3001: //心疗
		...
		{
			dam = CountHPPercent(pTargetData, 10, userlv);
			dam += 500;
			CorrectCureHP(dam, UserID, TargetID);//治疗修正
		}
		break;
	case 3002: //命疗
		...
		{
			dam = CountHPPercent(pTargetData, 15, userlv);
			dam += 800;
			CorrectCureHP(dam, UserID, TargetID);//治疗修正
		}
		break;
	case 3003: //凝气
		...
		{
			dam = CountMPPercent(pTargetData, 20, userlv);
			dam += 200;
	...
	case 3004:  //凝神
		...
		{
			dam = CountMPPercent(pTargetData, 30, userlv);
			dam += 400;
			CorrectCureMP(dam, UserID, TargetID);//治疗修正
		}
		break;
	...
	case 3049://迅疾如风
		...
		break;
	case 2102:
		CountphysicalDamage(dam, UserID, TargetID, userlv, 50);
		break;
	default:
		ERRBOX;
		break;
...
BOOL cSkillManager::Use1006(int Skilllv, int sSkillID, int UserID, int TargetID)
{
	...
	case 1:
		...
		break;
	case 2:
		if (rand() % 100 < 0 + g_pMainState->m_GlobalButton[13]+ pUserData->m_FuZhuSkillLvList[9] - pTargetData->m_FuZhuSkillLvList[10])
		{
			stage = 3;
			pUser->m_Wav.Free();
	...
	case 3:
	
		{
			...
			if (y2>480)
			{
				...
				if (UserID % 10 < 5)
				if (pUserData->m_Controler.CheckHaveBB())
					m_pFightState->m_pList[UserID+5]->isLive = FALSE;
...
{//法术命中
	...
	g_pMainState->m_FightList[TargetID].m_pData->GetDateP(pTargetData);
	int rate= pUserData->m_XiuLian.GetMag() - pTargetData->m_XiuLian.GetDMag();
	int offset = Skilllv- pTargetData->m_LV;
	if (offset > 0) offset = 0;
	int sealrate = pUserData->m_SealRate.d1 - pTargetData->m_SealRate.d2;
	int magichitrate = pUserData->m_MagicHitRate.d1 - pTargetData->m_MagicHitRate.d2;
	switch (sSkillID)
	{
		//大唐
	case 6:
	case 19://离魂
		rate += 35 + sealrate + offset; break;
		//方寸
	case 14://催眠
	case 15://失心
	case 16://落魄
	...
	case 20://失魂
	case 21://定身
		rate += 50 + sealrate + offset; break;
		//女儿
	case 54:rate += 20 + sealrate + offset; break;//似玉生香
	case 59:rate += 20 + sealrate + offset; break;
		//天宫
	case 67:rate += 25 + sealrate + offset; break;
	case 68:rate += 25 + sealrate + offset; break;
	case 69:rate += 75; break;
		//龙宫
	case 77:rate += 100; break;
	case 80:rate += 100; break;//龙卷
		//普陀五形法术
	case 2084:
	case 2085:
	case 2086:
	...
	case 108:
	case 109:
		rate += 90 + magichitrate; break;
		//魔王
	case 131:rate += 150 + magichitrate + offset; break;//飞沙
		//五庄
	case 89:rate += 35 + sealrate + offset; break;
		//地府
	case 122:rate += 40 + magichitrate + offset; break;
	case 127:rate += 80 + magichitrate + offset; break;
	case 129:rate += 40 + magichitrate + offset; break;
		//ST
	case 144:rate += 40 + sealrate + offset; break;
		//PS
	case 154:
	case 155:
	case 156:
	...
	case 164:
		if (1)
		{
			switch (sSkillID)
			{
			case 154:rate += 50 + offset; break;
			case 155:rate += 50 + offset; break;
			case 156:rate += 40 + offset; break;
			case 159:rate += 40 + offset; break;
			case 164:rate += 40 + offset; break;
			}	
			rate += (pUserData->m_Popularity - pTargetData->m_Popularity) / 50;
		}
		break;
	case 165:rate += 40 + magichitrate + offset; break;
	case 3:   //大唐   各种回门派
	case 25://方寸山
	case 44: //化生寺
	case 58: //女儿村
	...
	//rate += 5 +pUserData->m_MagicHitRate.d1 - pTargetData->m_MagicHitRate.d2;
	if ((rand() % 100) < rate)
		return TRUE;
	return FALSE;
}
...
{//五行伤害修正
	...
	case cSkillManager::eMWX光:
		offwx = pUserData->m_MagicThunder.d1 - pTargetData->m_MagicThunder.d2/5;
		offwx += pUserData->m_MagicThunder.d1 - pTargetData->m_MagicThunder.d2/5;
		offwx += pUserData->m_MagicThunder.d1 - pTargetData->m_MagicThunder.d2/5;
		offwx += pUserData->m_MagicThunder.d1 - pTargetData->m_MagicThunder.d2/5;
		offwx += pUserData->m_MagicThunder.d1 - pTargetData->m_MagicThunder.d2/5;
...
void cMengPaiManager::lvUpMainSkill(cPCData& pcDate, int lv, int ID)
{
	...
	int prelv = skilllv;
	int nextlv = prelv + lv;
	switch (pcDate.m_MengPaiID)//门派随等级增加资质
	{
	case 0://大唐
		switch (ID)
		{
