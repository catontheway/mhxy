...
BOOL CMainState::ProcessInterface()
{
	...
	/*if (Keyboard.GetKeyState(DIK_5))
	{
		Keyboard.SetLock(DIK_5);
		m_FightList[m_HeroID].m_pData->Set(POS_SUFFER);
		m_FightList[m_HeroID].m_pData->SetFrame(1);
		m_FightList[m_HeroID].m_pData->SetStatic(TRUE);
	}*/
	//
	
	//if (Keyboard.GetKeyState(DIK_2))//武器升级
	//{
	//	Keyboard.SetLock(DIK_2);
	//	sItem2& weappon = g_pCharacter->m_PCData.m_Equip[2];
	//	g_pCharacter->m_PCData.m_pCharacterData2 = 0;
	//	int type = weappon.GetType();
	//	
	//	int nextid = weappon.GetID();
	//	if (nextid < 27)nextid += 3;
	//	else nextid += 1;
	//	if (nextid>28)nextid = 0;
	//	weappon.Set(type,nextid );
	//	m_FightList[m_HeroID].m_pData->m_NowPos = -1;
	...
	if (g_StateType == STATE_MAIN)
	{
		...
		if (Keyboard.GetKeyState(DIK_P))
		{
			...
			m_InterfacePet2.OnOff(!m_InterfacePet2.m_NeedShow);
			return TRUE;
		}
		/*******************End 炼妖 合宠**********************/
		//R键按下
...
{
	pObj->SetDataType(INDEXTYEP_CHARACTER);
	cCharacter* pc = (cCharacter*)pObj->m_pIndex;
	cPCData& pcd = pc->m_PCData;
	pcd.Read(Path);
	pc->SetData(&m_pCharacterDataList[pc->m_PCData.modeid]);
	pObj->m_TrueName.SetColor(RGB(0, 255, 0));
	pObj->m_TrueName.SetString(pc->m_PCData.m_Name);
	cPetData* pPetData;
...
BOOL CMainState::ReadyToFight()
{
	...
	if (0 == m_Map.m_NumOfEmemyType)return TRUE;
 		...
 		for (int i = 10; i<10+num; i++)
		{
			...
			case 93:
			case 137:
				addpoint.Set(0, 0, 4, 1, 0); break;
			default:
				addpoint.Set(4, 0, 0, 1, 0); break;
			}
				/**/
			switch (BBType)
			{
			case 0:
				...
				{		
					MakeAPet(pPet, bbmode, 60, m_Map.m_EnemyLv + rand() % 3, addpoint);
					pObj->m_TrueName.SetString(pObj->GetAutoName());
					pPet->m_PetDataZZ.m_HPMax.SetData(pPet->m_PetDataZZ.m_HPMax.GetData() * 2);
			...
			case 1:
				
				{
					...
					MakeAPet(pPet, bbmode, 80, m_Map.m_EnemyLv + rand() % 3 + 2, addpoint);
			...
			case 2:
				...
				{
					MakeAPet(pPet, bbmode, 100, 0, addpoint);
					name2 = pObj->GetAutoName();
					name2 += "宝宝";
					pObj->m_TrueName.SetString(name2);
...
BOOL CMainState::SetFightPet(int ID, int PetID,BOOL blimit)
{
	...
	else     //设置参战
	{
		...
		//设置修炼
		pet->m_PetDataZZ.m_XiuLian.Set(&pet->m_PetDataZZ, pc->m_PCData.m_BBXiuLian);
...
void CMainState::Init3()
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		for (int j = 0; j < 5; j++)
			m_FightList[i].m_Damage[j].m_hFont = m_hFont[4];
	}
...
DWORD WINAPI cFightState::ThreadProc(_In_ LPVOID lpParameter)
{
...
BOOL cFightState::BackFromFight(int i)
{
	...
	pTarget->SetShake(FALSE);//有可能是在抖动时结束战斗
	cPetData* pTargetData;
	pTarget->GetDateP(pTargetData);
	//还原角色的位置,方向,并使之站立
...
BOOL cFightState::PrePareToFight(int i)
{
	...
	cPetData* pet;
	m_pList[i]->m_pData->GetDateP(pet);
	if (1 > pet->m_HP)
	{
		pet->m_HP = 1;
	}
		...
		if (!g_pMainState->m_InterfaceDoor.CheckWhite())
		{//夜战
			int nightoffset = pTargetData->nightLv - 20;
			pTargetData->m_ATK.AddDataAddPercent(nightoffset);
			pTargetData->m_DEF.AddDataAddPercent(nightoffset);
			pTargetData->m_Wakan.AddDataAddPercent(nightoffset);
		}
...
void cFightState::EndFight(BOOL bFriendCure)
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		m_pList[i]->m_pData->SetStatic(FALSE);
		ClearState(i);
		m_pList[i]->m_pData->GetDateP(pPetData);

		if (pPetData->m_Controler.GetDel())
		{
			g_pMainState->FreeFightList(i);
		}
		else
...
DWORD WINAPI cMenuState::ThreadProc(_In_ LPVOID lpParameter)
{
