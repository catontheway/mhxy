...
void cAttackAI::RunDT()
{
	...
	if (!m_pUserPetData->m_FightProperty.cannotMagic)
	{
		...
		if (m_pUserPetData->m_LV > 25)//大于25才会用横扫
		{
...
void cAttackAI::RunFC()
{
	...
	if (!m_pUserPetData->m_FightProperty.cannotMagic)
	{
		...
		if (m_FightTarget[m_UserID] == -1)
		{
			if (!m_pTarget->CheckHaveState(21))
				return UseMagic(21);//定身
		}
		else
		{
			//检测我方是物理型的多还是法术型的多
			...
			{
				if (rand() % 2 == 0)
					return UseMagic(28);//失心
				else
					return UseMagic(15);//失心
			}
			else return UseMagic(19);//离魂
		}
		if (!m_pUserPetData->m_FightProperty.cannotUseItem)
		{
...
void cAttackAI::RunHS()
{
	...
	//烧双专用
	if (g_pMainState->m_TriggerFightSuccess.m_TaskList[0].m_pTask->m_type == 0)
	{
		SetTarget(g_pMainState->m_AI.AutoFindTarget(m_UserID, 4));
		return UseMagic(31);	
	}
		...
		if (g_pMainState->m_FightList[10].isLive)
		{
			...
			if (ppetdata->m_HP > m_pUserPetData->m_LV * 200)
			{
				...
				if (!CheckCanSkill(72))
				{
					...
					if (CheckItem(eAIUseItemTypeMP)) { UseItem(eAIUseItemTypeMP); return; }
					if (CheckItem(eAIUseItemTypeMPALL)){ UseItem(eAIUseItemTypeMPALL); return; }
					if (m_pUserPetData->m_AddPoint.mp > 2)//魔天宫
						return UseMagic(71);
					return Def(m_UserID);
				}
				return UseMagic(72);
...
{	
	...
	if (!m_pUserPetData->m_FightProperty.cannotMagic)
	{
		...
		{
			if (m_pUserPetData->m_HP > m_pUserPetData->m_HPMax.GetData() * 50 / 100)//血气>50
			{//烟雨
...
//	int ailv = m_pUserPetData->m_AiLv;
	...
	case eMP_大唐官府://大唐怪物
			...
			case 1:
				...
				{
					if (m_pUserPetData->m_LV > 90)
					{
						m_pUserPetData->m_CP += 100;//CP＋100
						return NpcTryMagic(5);//万剑
					}
				}
				if (m_pUserPetData->m_FightProperty.cannotAttack)return NpcTryMagic(0);
				return NormalAtk();
				break;
			case 2:return NpcTryMagic(1);//后发
			case 3:
				if (!CheckHaveState(i, 4))
				{
					...
					{
						SetTarget(i);
						return UseMagic(4);//没有杀气诀使用杀气诀
					}
					else
						return NpcTryMagic(9);//有杀气诀用破釜沉舟
				}
				else
				{
					if (m_pUserPetData->m_FightProperty.cannotAttack)return NpcTryMagic(0);//不能攻击用横扫
					return NormalAtk();
				}
			case 4:
				if (m_pUserPetData->m_LV > 60)
					return NpcTryMagic(10);//长驱直入
				else
					return NpcTryMagic(9);
	...
	case eMP_方寸山://方寸怪物
		...
		case 1:
			if (1)
			{
				...
				switch (rand() % 4)
				{
				case 1:return NpcTryMagic(15);//失心
				case 2:return NpcTryMagic(19);//离魂
				case 3:return NpcTryMagic(17);//失忆
				default:return NpcTryMagic(21);//定身
				}

			}
		default:return NpcTryMagic(28);
		}
		break;
	case eMP_化生寺:
			switch (rand() % 7)
	...
	case eMP_天宫:
			...
			case 2:
				if (1)
				{
					...
					if (rand() % 2)
					{
						return NpcTryMagic(68);//错乱
					}
					else return NpcTryMagic(69);//百万
				}
				return;
			case 3:
				if (m_pUserPetData->m_FightProperty.cannotAttack)
				{
					return NpcTryMagic(71);//天雷
				}
				else
				return NormalAtk();
			case 4:return NpcTryMagic(72); 
			case 5: 
				if (!CheckHaveState(i, 62))
				{
					if (CheckCanSkill(62))
					{
						SetTarget(i);
						return UseMagic(62);//没有天神护体使用天神护体
					}
					else
					{
						if (m_pUserPetData->m_LV > 89)
							return NpcTryMagic(74);//等级大于89用雷霆
						else
