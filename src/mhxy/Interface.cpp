	...
	case 1:  //升级
		if (1)
		{
			...
			if (limitlv)
			{
				...
				{
					g_pMainState->m_Tags.Add("无法升级了");
					return FALSE;
				}
			}
			if (!m_pPCData->LeveUp())return TRUE;
...
//	File.close();
//	string keyStr = "abcdefghijklmnop";  //密钥
//	AES aes(keyStr);
//	aes.aesEncrypt(oss.str(), oss.str() + ".save");
//	remove(oss.str());
//}
...
void cInterfaceSystem::Save(int id)
{
	...
	ofstream File;
	string oss;
	oss = gStrMediaPath + "存档//5.txt";
	File.open(oss.c_str());
	File << id;
	File.close();
	oss = "";
	oss = gStrMediaPath + "存档//";
	oss += id + '0';
	oss += ".txt";
	...
	//AES aes(keyStr);
	encode myencode;
	myencode.filecode(oss.c_str(), oss + ".save");
	//remove(oss.c_str());
}
	...
	for (int i = 0; i < 20; i++)
	{
		...
		else
		{
			...
			case 24://法宝
				if (1)
				{
					switch (Item.m_pItemData->m_UseType)
					{
					case 0:
					case 1:
					case 7:
					...
					default:
						m_ItemCanUse[i] = TRUE;
						g_pMainState->m_InterfaceItem.m_ItemHead[i].Load(Item.m_pItemData->m_SmallHead);
						break;
					}
				}
				break;
...
void cInterfaceXiuLian::UpDate()
{
	...
	if (lv < 0)lv = 0;
	lv /= 5;//当前等级的最大修炼等级
	sXiuLian& liulian = g_pCharacter->m_PCData.m_XiuLian;
...
BOOL cItemSelect::Process(int xMouse, int yMouse)
{
	if (m_Describe.m_NeedShow)
		m_Describe.OnOff(FALSE);
...
void cItemSelect::Update()
{
	m_SelectMark.m_NeedShow = FALSE;
	for (int i = 0; i < 20; i++)
	{
		m_ItemNum[i] = g_pCharacter->m_PCData.m_Item[i].GetNum();
		if (m_ItemNum[i])
		{
			m_ItemHead[i].m_NeedShow = TRUE;
			m_ItemHead[i].Load(g_pCharacter->m_PCData.m_Item[i].m_pItemData->m_SmallHead);
			if (m_ItemNum[i] > 1)
			{
				ostringstream oss;
				oss << m_ItemNum[i];
...
void sShangPinPet::Load(ifstream& File)
{
	File >> m_modeid;
	File >> m_name;
	int num;
...
BOOL cInterfaceDaZao::CheckL()
{
	...
	case 5:
		if (m_MoneyNeed)
		{
			...
			if (0 == prop.size())
			{
				g_pMainState->m_Tags.Add("未鉴定你打造个毛线");
				return TRUE;
			}
			BOOL bSuccess = FALSE;
			string text;
...
void cInterfacePet2::Init()
{
	if (!m_bInit)
	{
		...
		oss << "合宠";
		m_TPetProPerty[0].SetString(oss.str());
		m_TPetProPerty[0].SetColor(RGB(250, 250, 250));
		oss.str("");
		oss << "炼妖";
		m_TPetProPerty[1].SetString(oss.str());
		m_TPetProPerty[1].SetColor(RGB(250, 250, 250));
		m_Cancel.Set(&g_pMainState->m_Cancel);
		m_Cancel.m_bStatic = TRUE;
		m_back.Load(0x16829301);
		m_Hechong.Load(0x79D01E0E);
		m_Hechong.SetStatic(TRUE);
		m_Lianyao.Load(0x79D01E0E);
		m_Lianyao.SetStatic(TRUE);
	}
	m_PetObjL.Init();
	m_PetObjL.SetDirection(3);
	m_PetObjL.m_IndexType = INDEXTYEP_PET;

	m_PetObjR.Init();
	m_PetObjR.SetDirection(3);
	m_PetObjR.m_IndexType = INDEXTYEP_PET;
	SetPage(0);
	SelectPet(-1);
	SelectPet(-2);
	Move(30, 50);
...
BOOL cInterfacePet2::Show(cCanvas* pCanvas)
{
	if (!m_NeedShow) { return TRUE; }
	SetPage(0);
	pCanvas->Add(&m_back, 1, TRUE);
	...
	m_TPetProPerty[0].SetColor(RGB(250, 250, 250));
	m_TPetProPerty[1].SetColor(RGB(250, 250, 250));
	pCanvas->Add(&m_Hechong, 1);
	pCanvas->Add(&m_Lianyao, 1);
	for (int i = 0; i < 2; i++)
	{
		pCanvas->Add(&m_TPetProPerty[i]);
	}
	...
	if (m_PetObjL.m_pIndex)
	{
		pCanvas->Add(m_PetObjL.GetShadow(), 1);
		m_PetObjL.ShowOnScreen(pCanvas);
	}

	if (m_PetObjR.m_pIndex)
	{
		pCanvas->Add(m_PetObjR.GetShadow(), 1);
		m_PetObjR.ShowOnScreen(pCanvas);
	}
	return TRUE;
}

BOOL cInterfacePet2::ProcessInput()
{
	if (m_Sleeptime > 0)
	{
		g_pMainState->SetMouseOnMap(FALSE);
	...
	{
		g_pMainState->SetMouseOnMap(FALSE);
		if (CheckOn(&m_Cancel, 30))return TRUE;
		if (CheckOn(&m_Hechong, 51))return TRUE;
		if (CheckOn(&m_Lianyao, 52))return TRUE;
		//m_TPetName[i].SetXY(m_xPos + 20, m_yPos + 190 + i * 20);
		if (g_yMouse > m_yPos + 190 && g_yMouse<m_yPos + 410 && g_xMouse>m_xPos + 20 && g_xMouse < m_xPos + 130)
		{
			int select = (g_yMouse - (m_yPos + 190)) / 20;
			m_Select = select;
			return CheckL();
		}
		//关闭界面
...
BOOL cInterfacePet2::Move(int x, int y)
{
	m_xPos = x;
	m_yPos = y;
	m_back.SetXY(x, y);
	m_Cancel.SetXY(m_back.GetX() + 420, m_back.GetY() + 5);

	m_Hechong.m_Picture.m_x = m_xPos + 175;
	m_Hechong.m_Picture.m_y = m_yPos + 114;

	m_Lianyao.m_Picture.m_x = m_xPos + 225;
	m_Lianyao.m_Picture.m_y = m_yPos + 114;
	...
	m_PetObjL.SetXY(m_xPos + 100, m_yPos + 130);
	m_PetObjL.MoveShadow();

	m_PetObjR.SetXY(m_xPos + 360, m_yPos + 130);
	m_PetObjR.MoveShadow();

	MovePetProPerty();
	return TRUE;
}

BOOL cInterfacePet2::MovePetProPerty()
{
	m_TPetProPerty[0].SetXY(m_xPos + 182, m_yPos + 116);
	m_TPetProPerty[1].SetXY(m_xPos + 232, m_yPos + 116);
	return TRUE;
}


cInterfacePet2::~cInterfacePet2()
{
	Free();
}

BOOL cInterfacePet2::Free()
{
	m_PetObjL.m_IndexType = 0;
	m_PetObjL.m_pIndex = 0;
	m_PetObjR.m_IndexType = 0;
	m_PetObjR.m_pIndex = 0;
	return TRUE;
}

BOOL cInterfacePet2::SelectPet(int id)
{
	for (int i = 0; i < 8; i++)
	{
		m_TPetName[i].SetString("");
		m_TPetName[i].SetColor(RGB(0, 0, 0));
	}
	if (!m_Flag&&id != m_SelectR)
	{
		m_SelectL = id;
		if (m_SelectL >= 0) {
			cPet* pPetSelect = g_pCharacter->m_PCData.m_pPetDataList[m_SelectL];
			m_PetObjL.m_pIndex = pPetSelect;
			m_PetObjL.m_IndexType = INDEXTYEP_PET;
			m_PetObjL.Reset();
			m_PetObjL.Set(POS_STAND);
		}
		else {
			m_PetObjL.m_pIndex = 0;
			m_PetObjL.m_IndexType = 0;
	...
	if (m_Flag&&id != m_SelectL)
	{
		m_SelectR = id;
		if (m_SelectR >= 0) {
			cPet* pPetSelect = g_pCharacter->m_PCData.m_pPetDataList[m_SelectR];
			m_PetObjR.m_pIndex = pPetSelect;
			m_PetObjR.m_IndexType = INDEXTYEP_PET;
			m_PetObjR.Reset();
			m_PetObjR.Set(POS_STAND);
		}
		else
		{
			m_PetObjR.m_pIndex = 0;
...
BOOL cInterfacePet2::SelectItem(int id)
{
	return 0;
}

BOOL cInterfacePet2::Hechong()
{
	m_Hechong.SetFrame(1);
	//判断是否可以合宠
	if (m_SelectL < 0 || m_SelectR < 0) {
		g_pMainState->m_Tags.Add("请选择两个召唤兽");
		return FALSE;
	}

	if (m_SelectR < m_SelectL) {
		int temp = m_SelectR;
		m_SelectR = m_SelectL;
		m_SelectL = temp;
	}
	...
	cPet* selectR = g_pCharacter->m_PCData.m_pPetDataList[m_SelectR];
	cPet* selectL = g_pCharacter->m_PCData.m_pPetDataList[m_SelectL];

	cPetData * petDataL = &selectL->m_PetDataZZ;
	vector<sSkill*> skillL = petDataL->m_pPetSkill;
	int numberOfSkillL = petDataL->m_NumofBBSkill;

	cPetData * petDataR = &selectR->m_PetDataZZ;
	vector<sSkill*> skillR = petDataR->m_pPetSkill;
	int numberOfSkillR = petDataR->m_NumofBBSkill;

	//创建新召唤兽
	cPet* newPet = 0;
	int newPetId = rand() % 1000;
	...
	case 161:
	{
		...
		g_pMainState->MakeAPet(newPet, newPetId, 100, 0, addpoint);
		cPetData * newPetData = &newPet->m_PetDataZZ;
		int modelid = newPetData->modeid;
		for (int i = 0; i < numberOfSkillL; i++) {
			g_pMainState->m_SkillManager.SetPetSkill(newPetData, skillL[i]->m_Id, newPetData->m_NumofBBSkill);
		}
		for (int i = 0; i < numberOfSkillR; i++) {
			g_pMainState->m_SkillManager.SetPetSkill(newPetData, skillR[i]->m_Id, newPetData->m_NumofBBSkill);
		}
		break;
	}
	default:
	{
		if (newPetId % 5 == 0)
		{
			...
			for (int i = 0; i < numberOfSkillL; i++) {
				if (rand() % 5 == 0) {
					g_pMainState->m_SkillManager.SetPetSkill(petDataR, skillL[i]->m_Id, petDataR->m_NumofBBSkill);
				}
			}
		}
		else
		{
			...
			for (int i = 0; i < numberOfSkillR; i++) {
				if (rand() % 5 == 0) {
					g_pMainState->m_SkillManager.SetPetSkill(petDataL, skillR[i]->m_Id, petDataL->m_NumofBBSkill);
				}
			}
		}
		break;
	...
	//移除消耗的召唤兽
	g_pMainState->SetFightPet(g_pMainState->m_HeroID, -1);  //召还

	if (deleteL) {
		g_pCharacter->m_PCData.DeletePet(m_SelectL);
		m_SelectR -= 1;
	}

	if (deleteR) {
		g_pCharacter->m_PCData.DeletePet(m_SelectR);
	}

	if (addNew) {
		//添加召唤兽
		g_pCharacter->m_PCData.GetPet(newPet);
	}

	g_pMainState->m_PCHead.UpdatePet();

	//成功
	g_pMainState->m_Tags.Add("合宠成功");
...
BOOL cInterfacePet2::Lianyao()
{
	m_Lianyao.SetFrame(1);
	//判断是否可以炼妖
	//移除消耗
	//更新召唤兽属性
	g_pMainState->m_Tags.Add("炼妖成功(功能未实现,敬请期待)");
	//OnOff(FALSE);
	return 0;
}

BOOL cInterfacePet2::SetPage(int page)
{
	for (int i = 0; i < 8; i++)
	{
		m_TPetName[i].SetString("");
	...
	int num2 = g_pCharacter->m_PCData.m_NumofBB - page * 8;
	if (num2 > 8) { num2 = 8; }
	for (int i = 0; i < num2; i++)
	{
		m_TPetName[i].SetString(g_pCharacter->m_PCData.m_pPetDataList[i + page * 8]->m_PetDataZZ.m_Name);
		m_TPetName[i].SetColor(RGB(0, 0, 0));
	}
	if (m_SelectL >= 0) {
		m_TPetName[m_SelectL].SetColor(RGB(255, 255, 0));
	}
	if (m_SelectR >= 0) {
		m_TPetName[m_SelectR].SetColor(RGB(255, 255, 0));
	}
	return 0;
}

void cInterfacePet2::OnOff(BOOL NeedShow)
{
	if (NeedShow == m_NeedShow)return;
	g_pMainState->InterfaceSetShow(this, NeedShow);
