    ...
    public unsafe class FastBitmap : IDisposable {
        ...
        public void CopyFromArray(int[] colors, bool ignoreZeroes = false) {
            if (colors.Length != Width * Height) {
                throw new ArgumentException(
                    @"The number of colors of the given array mismatch the pixel count of the bitmap", nameof(colors));
            }
        ...
        public static Bitmap SliceBitmap(Bitmap source, Rectangle region) {
            ...
            if (sliceRectangle.IsEmpty) {
                throw new ArgumentException(
                    @"The provided region must not lie outside of the bitmap's region completely", nameof(region));
            }

            var slicedBitmap = new Bitmap(sliceRectangle.Width, sliceRectangle.Height);
            CopyRegion(source, slicedBitmap, sliceRectangle
