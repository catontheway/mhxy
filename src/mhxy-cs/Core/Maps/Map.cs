...
namespace mhxy.Core.Maps {
    ...
    public class Map : ResourceBase {
        ...
        public Map(string fileName) {
            ...
            _fileName = fileName;
            _logger.Info($"Create Map:{fileName}");
        }
        ...
        public override void Load() {
            ...
            _logger.Info($"Begin Load Map:{_fileName}");
            try {
                ...
                using (var fs = new FileStream(_fileName, FileMode.Open)) {
                    ...
                    _logger.Info($"Flag(302E314D):{_flag},Width:{_width}" +
                    ...
                    for (var i = 0; i < _unitSize; i++) {
                        ...
                        _logger.Debug($"Unit:{i},Offset:{_unitOffsets[i]}");
                    }
                    ...
                    _logger.Info($"Mask Head Flag(70030000):{_maskFlag},Mask Size:{_maskSize}");
                    for (var i = 0; i < _maskSize; i++) {
                        fs.Read(buffer4, 0, 4);
                        _maskOffsets[i] = BitConverter.ToInt32(buffer4, 0);
                        _logger.Debug($"Mask:{i},Offset:{_maskOffsets[i]}");
                    }
                ...
                _logger.Error($"Error In Load Map:{_fileName}", e);
            }
            _logger.Info($"End Load Map:{_fileName}");
            _loaded = true;
        }
        ...
        private Mask ReadMask(FileStream fs, int index, int offSet) {
            ...
            _logger.Debug($"Read Mask({index}:{offSet}):StartX:{startX},StartY:{startY},Width:{width},Height:{height},Size:{size}");
        ...
        private Unit ReadUnit(FileStream fs, int index, int offSet) {
            ...
            _logger.Debug($"Read Unit({index}:{offSet}):RealOffset:{realOffset}");
            _logger.Debug($"UnitData(jpeg):Flag(47-45-50-4A):{jpeg.Flag},Size:{jpeg.Size}");//也可能是 32-47-50-4A,JPG
            _logger.Debug($"UnitData(cell):Flag(4C-4C-45-43):{cell.Flag},Size:{cell.Size}");
            _logger.Debug($"UnitData(brig):Flag(47-49-52-42):{brig.Flag},Size:{brig.Size}");
            return unit;
        }
